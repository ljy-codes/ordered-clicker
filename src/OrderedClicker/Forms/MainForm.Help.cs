...
public sealed partial class MainForm
{
    ...
    private sealed class UsageHelpDialog : Form
    {
        ...
        private string HelpText =>
            $"""
            一、按五步工作台配置
            依次完成“模式、方案、采点、检查、运行”。设置方案名称、总循环次数和轮间等待时间；总循环次数表示全部启用点位按顺序执行多少轮，轮间等待只发生在两轮之间。

            二、采集点击位置
            点击“采点模式”后，将鼠标移动到目标位置并按 {_captureHotKeyText}。也可以点击“2 秒后记录”，利用倒计时切回目标窗口。每次记录会检查附近重复点位，删除、清空和新增后可撤销。采点完成后点击“结束采点”。
            ...
            开始前会显示总行数、启用行、禁用行、循环数、计划点击数和预计耗时，确认后有 3 秒倒计时。只有实际完成数与计划完全一致，程序才会显示全部完成。停止或异常后可从可靠断点继续，也可重新开始。

            六、快捷键与安全角
            默认快捷键为 F6 采点、F7 开始/暂停、F8 停止。点击“设置”可选择“单键快捷键”“组合快捷键”，也可直接按键自定义。保存时若被其他软件占用，程序会保留原快捷键并提示冲突。
            执行前必须启用安全角。将鼠标移入所选屏幕角并连续停留设定时间会停止任务；点位与安全角重叠时程序会拒绝执行。

            七、保存、打开和迁移
            2.1.0 使用 v4 .oclick 方案，并与 2.0.0 方案兼容。方案下拉会列出程序默认方案目录中的 .oclick 文件，也可以直接输入新的方案名称。
            打开方案目录：点击方案下拉右侧的文件夹按钮，可打开 %LocalAppData%\OrderedClicker\profiles。
            切换保护：当前方案有修改时，选择另一个本机方案会出现“保存 / 不保存 / 取消”。保存后切换、不保存直接切换、取消则留在当前方案。
            “保存”覆盖当前已绑定方案；“另存为”创建新的方案身份；“打开方案”可打开任意位置的 v4 .oclick 文件。
            “迁移旧方案”只读取 v1-v3 JSON 并转换为未保存的新方案，原文件不会被覆盖，也不会导出回旧格式。
            程序自动维护活动草稿。异常退出后下次启动可恢复；正常关闭且没有未保存修改时会清理草稿。

            八、云桌面增强
            在浏览器里操作云桌面时，勾选“云桌面增强”，点击“校准云桌面区域”，依次将鼠标移到远程画面的左上角、右下角并按 {_captureHotKeyText}。之后采集的点保存为区域内相对坐标。浏览器窗口移动或改变大小后，重新校准会保留有效相对坐标，只补齐缺失值。
            可勾选“等待画面稳定”。远程页面持续加载并超过设定时间时，程序会暂停，确认页面可操作后按 {_startPauseHotKeyText} 继续，或按 {_stopHotKeyText} 停止。

            九、显示器与屏幕缩放
            程序使用 Per-Monitor V2 DPI 和屏幕物理坐标处理点击位置。如果显示器分辨率、排列位置、主副屏关系或缩放比例发生变化，原坐标可能不再准确。出现相关提示时，请在当前显示环境下重新采点或重新校准。

            十、复杂流程建议
            开始前重点核对启用行数、计划点击数、预计耗时和安全停止方式。先用 1 轮验证完整流程，再增加循环次数。步骤较多时建议开启画面稳定等待；执行日志保存在应用数据目录的 logs 子目录，诊断信息保存在 diagnostics 子目录。
            """;
