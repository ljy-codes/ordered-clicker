...
public sealed partial class MainForm
{
    ...
    private sealed class UsageHelpDialog : Form
    {
        ...
        private string HelpText =>
            ...
            七、保存、导入和导出
            方案下拉：点击“方案名称”右侧箭头，会列出程序默认方案目录中的 JSON 文件；也可以直接输入新的方案名称。
            ...
            “保存”覆盖当前已绑定的本机方案；“另存为”选择新的 JSON 文件位置，并作为后续保存目标。
            “导入方案”用于默认目录以外的 JSON 文件。程序会先校验并显示名称、点位数和循环次数，确认后导入为副本，不会覆盖来源文件；首次保存会要求另存为。
            “导出方案”将当前完整方案写入指定 JSON 文件，适合备份或分享，并且不会改变当前方案的保存位置。
