...
def main() -> None:
    ...
    replacements = {
        ...
        "下次直接加载，无需重新填写参数。": "下次可导入方案副本，无需重新填写参数。",
        ...
        "07 保存与加载方案": "07 保存、导入与导出",
        "保存并重新加载方案": "保存并导入方案副本",
        "顶部“保存”和“加载”按钮。": "顶部“保存”“另存为”“导入方案”和“导出方案”按钮。",
        "保存方案，清空点位，再点击加载并选择保存的 JSON 文件。": "先另存为或导出方案，清空点位，再点击导入方案并选择该 JSON 文件。",
        "显示器环境改变后，即使加载成功也建议重新采点。": "显示器环境改变后，即使导入成功也建议重新校准或采点。",
        "直接编辑 JSON 导致格式损坏，加载失败。": "直接编辑 JSON 导致格式损坏，导入失败。",
        "<h2>保存与加载方案</h2>": "<h2>保存、导入与导出方案</h2>",
        "<h3>加载方案</h3>": "<h3>导入与导出方案</h3>",
        "点击“加载”，选择之前保存的 JSON 文件。加载成功后先检查点位数量、启用状态和参数，\n"
        "            再使用一轮测试确认坐标。加载损坏文件时，程序会提示错误，不会覆盖当前点位。":
        "点击“导入方案”，选择 JSON 文件并确认摘要。导入内容作为本机副本，来源文件不会被覆盖。\n"
        "            首次保存会使用“副本”文件名并要求另存为，且不能选择来源文件本身。\n"
        "            “导出方案”用于备份或分享当前完整方案，不会改变当前保存位置。导入失败或取消时，当前点位保持不变。",
        "<strong>加载方案失败</strong>": "<strong>导入方案失败</strong>",
        "确认选择的是连点器保存的 JSON 文件。不要手工删除字段或加入注释；损坏文件需要重新保存。":
        "确认选择的是连点器导出的 JSON 文件。不要手工删除字段或加入注释；损坏文件需要重新导出。",
    }
    for old, new in replacements.items():
        html = html.replace(old, new)

    html = re.sub(
        r'\s*<div class="quick-links" aria-label="快速入口">.*?</div>',
        "",
        html,
        count=1,
        flags=re.DOTALL,
    ...
    cloud_section = """
      <section id="cloud-desktop">
        ...
        <div class="panel">
          <h3>方案下拉与默认目录</h3>
          <ul>
            <li><strong>方案下拉：</strong>点击“方案名称”右侧箭头，只列出程序默认方案目录中的 JSON 文件；输入框仍可直接填写新名称。</li>
            <li><strong>打开方案目录：</strong>点击下拉框右侧的文件夹按钮，打开 <code>%LocalAppData%\\OrderedClicker\\profiles</code>。</li>
            <li><strong>切换保护：</strong>当前方案有修改时，选择另一个本机方案会出现“保存 / 不保存 / 取消”。保存后切换、不保存直接切换、取消则留在当前方案。</li>
            <li>默认目录以外的 JSON 文件继续使用“导入方案”，并按副本处理。</li>
          </ul>
        </div>
        <div class="panel">
          <h3>保存、另存为、导入和导出</h3>
          <p>“保存”覆盖当前绑定方案；“另存为”选择新的 JSON 路径并作为后续保存目标；“导入方案”校验并确认后导入为副本，首次保存会要求另存为并禁止覆盖来源；“导出方案”生成备份或分享文件，但不改变当前保存位置。</p>
        </div>
      </section>
"""
    html = html.replace(
