...
Windows 10/11 x64 免费开源桌面连点器，无授权码和联网验证。支持有序点位、批量时间设置、单点独立参数、总循环次数、可配置全局热键、DPI 缩放及多显示器。

![有序连点器主界面](artifacts/ordered-clicker-ui-v1.2.0.png)

## 下载与运行

普通用户请从仓库的 **Releases** 页面选择以下版本：

- 安装版：`ordered-clicker-setup-v1.2.0.exe`，双击后按向导安装，适合日常使用。
- 便携版：`ordered-clicker-portable-v1.2.0.zip`，解压后直接运行，适合临时使用。
...
1. 打开 `有序连点器.exe`。
2. 点击“采点模式”。
3. 将鼠标移动到目标位置，按 `Ctrl+Alt+F8` 依次记录多个点。
4. 在“点位时间”栏设置点击间隔或点后等待，点击对应“应用全部”可批量修改所有点。
5. 在表格中单独修改特殊点位；单行修改只对当前点生效。
6. 新采集点会继承“点位时间”栏中的当前数值。
7. 设置总循环次数和轮间等待时间。
8. 按 `Ctrl+Alt+F9` 开始，运行过程中再次按该组合键可暂停或继续。
9. 按 `Ctrl+Alt+F10` 可随时停止。
...
- `Ctrl+Alt+F8`：采点模式开启时记录当前鼠标位置。
- `Ctrl+Alt+F9`：开始、暂停或继续。
- `Ctrl+Alt+F10`：立即停止。

在“⚙ 设置”中点击快捷键输入框后，直接按下新的组合键即可修改。快捷键必须包含 `Ctrl`、`Alt`、`Shift` 或 `Win`，三个操作不能使用相同组合。可以点击“恢复默认快捷键”恢复上述默认值。
...
- 运行时用户主动移动鼠标可能影响点击结果，默认按 `Ctrl+Alt+F10` 可紧急停止。
