...
最终产品统一放在项目外层的 `交付产品\有序连点器` 文件夹中。最终产品目录只包含安装版 EXE、免安装 EXE、PDF 使用说明和 HTML 使用说明。ZIP 便携包属于内部构建产物，不放入最终产品文件夹。
...
pwsh -NoProfile -File .\scripts\build-installer.ps1 `
  -Version 1.3.1 `
  -GuideSourceDirectory "D:\专用工具\连点器\开发空间\操作指导"
```

应用发布结果位于 `publish\win-x64`，安装器和 ZIP 便携包中间产物位于 `publish\packages`。整理后的对外交付副本位于项目外层的 `交付产品\有序连点器`，只发布安装版 EXE、免安装 EXE、PDF 和 HTML。
