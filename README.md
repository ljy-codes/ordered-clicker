...
Windows 10/11 x64 免费开源桌面连点器，无授权码和联网验证。支持有序点位、批量时间设置、单点独立参数、总循环次数、全局热键、DPI 缩放及多显示器。

![有序连点器主界面](artifacts/ordered-clicker-ui-v1.1.0.png)

## 下载与运行

普通用户请从仓库的 **Releases** 页面选择以下版本：

- 安装版：`ordered-clicker-setup-v1.1.0.exe`，双击后按向导安装，适合日常使用。
- 便携版：`ordered-clicker-portable-v1.1.0.zip`，解压后直接运行，适合临时使用。
...
7. 设置总循环次数和轮间等待时间。
8. 按 `F9` 开始，运行过程中再次按 `F9` 可暂停或继续。
9. 按 `F10` 可随时停止。
10. 点击“⚙ 设置”可切换界面主题，点击“? 使用说明”可查看完整操作细节。
